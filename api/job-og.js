...
module.exports = async (req, res) => {
  ...
  try {
    if (id) {
      const r = await fetch(SUPABASE_URL + '/rest/v1/jobs?id=eq.' + encodeURIComponent(id) + '&status=eq.open&select=role,location,salary_range,description,hide_salary,industry', { headers: { apikey: ANON, Authorization: 'Bearer ' + ANON } });
  ...
  if (job) {
    ...
    const bits = [job.industry, job.location, (job.salary_range && !job.hide_salary) ? (job.salary_range + '/mo') : null].filter(Boolean).join(' · ');
